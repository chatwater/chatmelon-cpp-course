...
int main() {
    freopen("title.in", "r", stdin); //定义文件title.in为输入流
    freopen("title.out", "w", stdout); //定义文件title.out为输出流
    string s;
    int ans = 0;
    while (cin >> s) {
 ...
 * 输出2：（一个英文等于1个字符长度，所有11个字符长度，看来中间的空格没有算，当作的输入了2个字符串Hello和world! 它们的长度和 5+6=11）
 * title.out文件内容：11
 * 
 * 输入2:
 * title.in文件内容：我爱中国
